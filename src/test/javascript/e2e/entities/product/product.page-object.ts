...
export class ProductUpdatePage {
  ...
  recommendedSelect = element(by.id('field_recommended'));
  ...
  async setRecommendedSelect(recommended: string): Promise<void> {
    await this.recommendedSelect.sendKeys(recommended);
  }

  async getRecommendedSelect(): Promise<string> {
    return await this.recommendedSelect.element(by.css('option:checked')).getText();
  }

  async recommendedSelectLastOption(): Promise<void> {
    await this.recommendedSelect.all(by.tagName('option')).last().click();
  }
